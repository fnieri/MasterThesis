\section{An overview of $\Delta$QSD}
 ...
 \subsection{Outcome}
        ...
        Formally, what the system obtains by performing one of its tasks. One task corresponds to one outcome and viceversa. When an outcome is performed, it means that the task of an outcome is performed. \cite{myo}
...
\subsection{Quality attenuation ($\Delta$Q)}
  ...
  \subsection{Failure semantics}
    ...
    \subsection{Independence hypothesis}    
        ...
        When the system is far from being overloaded, the effect is noticeable thanks to $\Delta$QSD even if the system is far from being overloaded. As the cliff edge of overload is approached, the nonlinearity will increase. \cite{post}
