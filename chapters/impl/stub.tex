\section{Wrapper}
    ...
    \subsection{API}
        ...
        Moreover, the wrapper will always start OpenTelemetry spans but only start custom spans if the stub has been activated. The wrapper can be activated by the oscilloscope by pressing the "start wrapper" button and can be stopped via the "stop wrapper" button.
...
-spec fail_span( pid() | ignore) -> ok | term().
        ...
        \begin{itemize}
            ...
            \item \texttt{\{timeout, end\_time(StartTime + $dMax$)\}}: If the program hasn't ended the span before $dMax$, the timer will send a \texttt{timeout} message and it will send a custom span to the oscilloscope indicating that an execution of a probe has taken $> dMax$.
        \end{itemize}
