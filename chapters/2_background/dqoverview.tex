  ...
  \subsection{Failure semantics}
   ...
   \begin{figure}[H]
    ...
    \subsection{Independence hypothesis}  
        ...
        At low load, the two components behavior will be independent, the system will behave \textbf{linearly}. According to the superposition principle, the overall delay will be the sum of the two delays, as will the processor utilisation. \cite{sup-p}
        
        When load increases, the two components will start to show dependent behaviour due to the processor utilisation increasing. The $\Delta$Q of the observed total delay will then deviate from the sum of the two delays ($\Delta$Q$_1 \circledast \Delta$Q$_2$). 
        ...
        \begin{figure}[H]
            ...
            When $o_1$ and $o_2$ show initial signs of dependency, the sum of the two delays (green) can be seen deviating from the overall delay.}
            \label{fig:cdf_indep}
        \end{figure}
