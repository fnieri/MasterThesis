...
\node[rectangle,draw, minimum height = 2cm, minimum width = 1cm, right of = sut, anchor = west, xshift = 0.2cm] (stub) {$\Delta$Q Adapter};
    ...
    \draw [->] (5, 3) -- ([xshift=-1.5cm]osc.north) node[pos=0.1, above] {outcome diagram};

    \draw [->] (9, 3) -- ([xshift=0.5cm]osc.north) node[pos=0.1, above] {parameters (incl. triggers)};

    \draw[->] (osc.east) -- (10, 0) node[right] {$\Delta$Q display};
...
node [pos=0.5,anchor=north,yshift=-3cm] {Erlang};
...
node [pos=0.5,anchor=north,yshift=-3cm] {C++};
